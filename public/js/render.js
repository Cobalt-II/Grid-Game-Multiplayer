...
requestAnimationFrame(function draw() {
  ...
  for (let count = -player.fov; count < player.fov; count++) {
    for (let coun = -player.fov; coun < player.fov; coun++) {
      ...
      if (!k) {
        ctx.fillRect(
          (window.innerWidth * (count - -player.fov)) / (player.fov * 2),
          (window.innerHeight * (coun - -player.fov)) / (player.fov * 2),
          gridX,
          gridY
        );
      }
      for (let t in enemies) {
        if (
          enemies[t].pos[0] === player.x + count &&
          enemies[t].pos[1] === player.y + coun
        ) {
      ...
      for (let t in players) {
        ...
        if (
          players[t].x + players[t].px === player.x + count &&
          players[t].y + players[t].py === player.y + coun &&
          player.id !== players[t].id
        ) {
          ctx.fillRect(
            window.innerWidth / 2 -
              (player.x - players[t].x) * gridX +
              gridX * players[t].px,
            window.innerHeight / 2 -
              (player.y - players[t].y) * gridY +
              gridY * players[t].py,
            gridX,
            gridY
          );
        ...
        ctx.fillRect(
          window.innerWidth / 2 + player.px * gridX,
          window.innerHeight / 2 + player.py * gridY,
          gridX,
          gridY
        );

        if (
          players[t].x === player.x + count &&
          players[t].y === player.y + coun &&
          player.id !== players[t].id
        ) {
          ...
          ctx.fillText(
            players[t].name,
            window.innerWidth / 2 + count * gridX,
            window.innerHeight / 2 + coun * gridY - gridY
          );
          ctx.fillText(
            players[t].score,
            window.innerWidth / 2 + count * gridX,
            window.innerHeight / 2 + coun * gridY - gridY / 2
          );
        }
        if (
          players[t].health < players[t].maxhealth &&
          players[t].id !== player.id
        ) {
          ctx.fillStyle = localStorage.roomColors.split(",")[5];
          ctx.fillRect(
            window.innerWidth / 2 - (player.x - players[t].x) * gridX,
            window.innerHeight / 2 -
              (player.y - players[t].y) * gridY +
              bar.offset,
            gridX,
          ...
          ctx.fillRect(
            ...
            window.innerHeight / 2 -
              (player.y - players[t].y) * gridY +
              bar.offset,
            Math.max((players[t].health / players[t].maxhealth) * gridX, 0),
            bar.size[1]
          );
        }
        if (player.health < player.maxhealth) {
          ...
          ctx.fillRect(
            window.innerWidth / 2,
            window.innerHeight / 2 + bar.offset,
            gridX,
            bar.size[1]
          );
          ctx.fillStyle = localStorage.roomColors.split(",")[6];
  ...
  if (player.dead) {
    ...
    ctx.fillText(`Score: ${player.score}`, 0, font.size * 3 + 10);
  }
